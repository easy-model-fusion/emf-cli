...
func TestCompress(t *testing.T) {
	...
	err = os.WriteFile(filepath.Join(dir, "test.txt"), []byte("test"), os.ModePerm)
	if err != nil {
		t.Fatal(err)
	}

	// Compress the directory
	err = Compress(dir, filepath.Join(dir, "test.tar.gz"))
	if err != nil {
		t.Fatal(err)
	}

	// Compression should have created a file
	_, err = os.Stat(filepath.Join(dir, "test.tar.gz"))
	if err != nil {
		t.Fatal(err)
	}

	// Compress the file test
	err = Compress(filepath.Join(dir, "test.txt"), filepath.Join(dir, "test2.tar.gz"))
	if err != nil {
		t.Fatal(err)
	}

	// Compression should have created a file
	_, err = os.Stat(filepath.Join(dir, "test2.tar.gz"))
	if err != nil {
		t.Fatal(err)
	}

	// Compress a non-existing file
	err = Compress(filepath.Join(dir, "notFound"), filepath.Join(dir, "test3.tar.gz"))
	if err == nil {
		t.Fatal("Expected error")
	}

	// Compression should not have created a file
	_, err = os.Stat(filepath.Join(dir, "test3.tar.gz"))
	if err == nil {
